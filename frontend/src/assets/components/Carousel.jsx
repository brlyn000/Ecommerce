...
export default function Carousel() {
  ...
  if (carouselData.length === 0) {
    return (
      <div className="relative max-w-7xl mx-auto overflow-hidden rounded-2xl shadow-2xl">
        <div className="h-64 sm:h-80 md:h-96 lg:h-[32rem] bg-gradient-to-r from-red-500 to-purple-600 flex items-center justify-center">
      ...
      <div className="relative h-64 sm:h-80 md:h-96 lg:h-[32rem]">
        ...
        <div className="relative h-full w-full">
          {carouselData.map((item, index) => {
            const imageUrl = item.image?.startsWith('http') ? item.image.replace(':5005', ':5006') : item.image;
            return (
              <div
                key={index}
        ...
        <div className={`absolute bottom-8 left-8 z-20 text-white max-w-md transition-all duration-500 ${
          ...
          {carouselData[currentIndex]?.button_text && (
            <a 
              ...
              className="inline-block bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors duration-300 shadow-lg"
            >
              {carouselData[currentIndex]?.button_text}
            </a>
          )}
